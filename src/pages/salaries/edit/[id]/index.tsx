...
import { getSalaryById, updateSalaryById } from 'apiSdk/salaries';
...
function SalaryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<SalaryInterface>(
    () => (id ? `/salaries/${id}` : null),
    () => getSalaryById(id),
  );
  ...
  const handleSubmit = async (values: SalaryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSalaryById(id, values);
      mutate(updated);
      resetForm();
