...
import { getSalaries } from 'apiSdk/salaries';
...
import { getEmployeeDetails } from 'apiSdk/employee-details';
...
function DashboardPage() {
  ...
  const salaryStat = useSWR<PaginatedInterface<SalaryInterface>>(
    () => `/salaries`,
    () => getSalaries({ limit: 1, offset: 0 }),
  );
  ...
  const employeeDetailsStat = useSWR<PaginatedInterface<EmployeeDetailsInterface>>(
    () => `/employee-details`,
    () => getEmployeeDetails({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Salaries',
      result: salaryStat?.data?.totalCount,
      isLoading: salaryStat.isLoading,
    ...
    },
    {
      ...
      result: employeeDetailsStat?.data?.totalCount,
      isLoading: employeeDetailsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
